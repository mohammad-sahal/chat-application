...
const ChatArea = ({ socket, chat, chatType, onInitiateCall }) => {
    ...
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
        ...
        {!loading && !error && messages.map((message, index) => (
          ...
          >
            <div 
              className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${message.sender._id === user.id ? 'bg-blue-500 text-white' : 'bg-white text-gray-700'}`}
            >
              {message.type === 'voice' ? (
                <div className="flex items-center space-x-3">
                  <div className="flex items-center space-x-3 px-4 py-3 bg-white rounded-xl shadow-sm border border-gray-100 min-w-48">
                    <button
                      onClick={() => {
      ...
      <div className="p-4 bg-white border-t border-gray-200 relative">
            ...
            <div className="bg-white px-4 py-2 rounded-lg shadow-lg border border-gray-200">
              <div className="flex items-center space-x-2">
                ...
                <p className="text-gray-800 font-medium text-xs">
                  {chatType === 'user' 
                    ? `${chat.username || chat.name} is typing...`
                    : `${messages.find(msg => msg.sender._id === typingUser)?.sender.username || 'Someone'} is typing...`
                  }
                </p>
              </div>
            </div>
        ...
        <form onSubmit={handleSendMessage} className="flex items-center space-x-2 bg-gray-50 rounded-2xl p-2 shadow-sm border border-gray-200">
          ...
          <button
            ...
            className="voice-btn p-3 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all duration-200 flex-shrink-0"
            title="Voice Message"
          >
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none px-3 py-2 text-gray-700 placeholder-gray-500 focus:ring-0"
            disabled={loading}
          />
          ...
          <button
            ...
            className="emoji-btn p-3 text-gray-500 hover:text-yellow-600 hover:bg-yellow-50 rounded-xl transition-all duration-200 flex-shrink-0"
            title="Emoji (Ctrl+E)"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
          ...
          <button
            ...
            className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-3 rounded-xl hover:from-blue-600 hover:to-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
